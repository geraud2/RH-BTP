...
import { 
  ...
  Search, Filter, X, HardHat, Wrench, Truck, 
  Smartphone, User, MapPin, Calendar, CheckCircle2,
  ShieldAlert, Archive
} from 'lucide-react';
...
import { mockData, typesMateriel, chantiers } from '../../data/mockDataRH';
...
const typeConfig: Record<string, { icon: typeof HardHat; color: string; bg: string; label: string }> = {
  'EPI': { icon: HardHat, color: 'text-amber-600', bg: 'bg-amber-50', label: 'EPI' },
  'Outillage': { icon: Wrench, color: 'text-sky-600', bg: 'bg-sky-50', label: 'Outillage' },
  'Véhicule': { icon: Truck, color: 'text-emerald-600', bg: 'bg-emerald-50', label: 'Véhicule' },
  'Équipement': { icon: Smartphone, color: 'text-violet-600', bg: 'bg-violet-50', label: 'Équipement' },
};
...
export default function MaterielTravail() {
  ...
  const [newMat, setNewMat] = useState({ 
    type: typesMateriel[0], designation: '', employeeName: '', chantier: chantiers[0] 
  });
  ...
  const handleAdd = () => {
    ...
    const mat: Materiel = {
      ...
      chantier: newMat.chantier,
      etat: 'Bon',
    };
    setMateriels([mat, ...materiels]);
    setShowAdd(false);
    setNewMat({ type: typesMateriel[0], designation: '', employeeName: '', chantier: chantiers[0] });
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5 sm:space-y-6 max-w-2xl mx-auto pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Matériel de travail</h1>
          <p className="text-gray-500 text-xs sm:text-sm mt-0.5">Attribution et suivi du matériel chantier</p>
        </div>
        <div className="flex items-center gap-2">
          <Package size={18} className="text-amber-600" />
          <span className="text-sm font-semibold text-gray-700">{stats.total} équipements</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
        ...
        <div className="bg-white border border-emerald-200 rounded-2xl p-3 sm:p-4">
          ...
          <p className="text-lg sm:text-xl font-extrabold text-emerald-600">{stats.bon}</p>
          <p className="text-[10px] sm:text-xs text-emerald-600 font-medium">Bon état</p>
        </div>
        <div className="bg-white border border-amber-200 rounded-2xl p-3 sm:p-4">
          <div className="w-9 h-9 rounded-xl bg-amber-50 flex items-center justify-center mb-2">
            <AlertTriangle size={18} className="text-amber-600" />
          </div>
          <p className="text-lg sm:text-xl font-extrabold text-amber-600">{stats.remplacer}</p>
          <p className="text-[10px] sm:text-xs text-amber-600 font-medium">À remplacer</p>
        </div>
        <div className="bg-white border border-red-200 rounded-2xl p-3 sm:p-4">
          <div className="w-9 h-9 rounded-xl bg-red-50 flex items-center justify-center mb-2">
            <ShieldAlert size={18} className="text-red-600" />
          </div>
          <p className="text-lg sm:text-xl font-extrabold text-red-600">{stats.perdu + stats.reparation}</p>
          <p className="text-[10px] sm:text-xs text-red-600 font-medium">Perdu/Réparation</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Rechercher un matériel ou employé..."
          ...
          />
          {searchTerm && (
            <button onClick={() => setSearchTerm('')} className="absolute right-2 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-100 rounded-lg">
        ...
        <Button size="sm" onClick={() => setShowAdd(true)} className="flex-shrink-0">
          <Plus size={16} /> <span className="hidden sm:inline ml-1">Attribuer</span>
        </Button>
      </div>
      ...
      {showFilters && (
        <div className="flex flex-wrap gap-2 p-4 bg-white border border-gray-100 rounded-2xl">
          ...
          <select value={filtreEtat} onChange={(e) => setFiltreEtat(e.target.value)} className="px-3 py-2 text-xs rounded-xl border border-gray-200 bg-white flex-1 min-w-[140px]">
            ...
            <option value="À remplacer">⚠️ À remplacer</option>
            <option value="Manquant/Perdu">❌ Manquant/Perdu</option>
            <option value="En réparation">🔄 En réparation</option>
          </select>
          {hasActiveFilters && (
            <button
              onClick={() => { setFiltreType(''); setFiltreEtat(''); }}
      ...
      {/* Statistiques attribution */}
      <div className="flex items-center gap-2 text-xs text-gray-500">
        <User size={14} />
        <span>{stats.attribues} matériels attribués sur {stats.total}</span>
        <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-amber-500 rounded-full"
            style={{ width: `${stats.total > 0 ? (stats.attribues / stats.total) * 100 : 0}%` }}
          />
        </div>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-gray-500 font-medium">Aucun matériel trouvé</p>
            <p className="text-xs text-gray-400 mt-1">Essayez de modifier les filtres</p>
          </div>
        ) : (
          filtered.map((m) => {
            const typeConfigItem = typeConfig[m.type] || { icon: Package, color: 'text-gray-600', bg: 'bg-gray-50' };
            const TypeIcon = typeConfigItem.icon;
            const StatutIcon = etatIcon[m.etat];

            return (
              <div
                key={m.id}
                className={`bg-white border rounded-2xl p-3 sm:p-4 transition-all hover:shadow-sm ${
                  m.etat === 'Bon' ? 'border-gray-100' :
                  m.etat === 'À remplacer' ? 'border-amber-200 bg-amber-50/20' :
                  m.etat === 'Manquant/Perdu' ? 'border-red-200 bg-red-50/20' :
                  'border-gray-200 bg-gray-50/50'
                }`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${typeConfigItem.bg}`}>
                      <TypeIcon size={20} className={typeConfigItem.color} />
                    </div>
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="text-sm font-bold text-gray-900 truncate">{m.designation}</p>
                        <Badge variant={etatVariant[m.etat]}>
                          <StatutIcon size={10} className="mr-1 inline" />
                          {m.etat}
                        </Badge>
                      </div>
                      <div className="flex items-center gap-2 text-xs text-gray-500 flex-wrap">
                        ...
                        <span className="flex items-center gap-1">
                          <MapPin size={11} /> {m.chantier}
                        </span>
                      </div>
                      {m.employeeName ? (
                        <div className="flex items-center gap-2 mt-1.5 text-xs text-gray-500">
                          <span className="flex items-center gap-1">
                            <User size={11} /> {m.employeeName}
                          </span>
                          <span className="text-gray-300">•</span>
                          <span className="flex items-center gap-1">
                            <Calendar size={11} /> {m.dateAttribution}
                          </span>
                        </div>
                      ) : (
                        <p className="text-xs text-gray-400 mt-1.5 italic">Non attribué</p>
                      )}
                    </div>
                  </div>
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Attribuer du matériel" size="md">
        <div className="space-y-4">
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Type de matériel</label>
            <select
              value={newMat.type}
          ...
          <Input 
            label="Désignation *" 
            value={newMat.designation} 
            onChange={(e) => setNewMat({ ...newMat, designation: e.target.value })}
            placeholder="Ex: Casque de sécurité jaune"
          />
          <Input 
            label="Attribué à" 
            value={newMat.employeeName} 
            onChange={(e) => setNewMat({ ...newMat, employeeName: e.target.value })}
            placeholder="Nom de l'employé"
          />
          <div>
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Chantier</label>
            <select
              value={newMat.chantier}
              onChange={(e) => setNewMat({ ...newMat, chantier: e.target.value })}
              className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none"
            >
              {chantiers.map(c => <option key={c} value={c}>🏗️ {c}</option>)}
            </select>
          </div>
          <Button onClick={handleAdd} className="w-full" disabled={!newMat.designation}>
            <Plus size={16} /> Attribuer le matériel
          </Button>
        </div>
      </Modal>
    </div>
